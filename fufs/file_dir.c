...
static bool dir_locate(device_handle_t device, fufs_filesystem_t* fs, inode_no_t dir, const char* name, bool* p_exist, inode_no_t* p_no);
static bool dir_add(device_handle_t device, fufs_filesystem_t* fs, inode_no_t dir, const char* name, inode_no_t no);
static bool dir_del(device_handle_t device, fufs_filesystem_t* fs, inode_no_t dir, const char* name);
static bool dir_tree_locate(device_handle_t device, fufs_filesystem_t* fs, inode_no_t start, const char* relative_path, bool* p_exist, inode_no_t* p_no);
static bool dir_roottree_locate(device_handle_t device, fufs_filesystem_t* fs, const char* path, bool* p_exist, inode_no_t* p_no);
...
static bool fufs_file_init(fufs_file_t* file, device_handle_t device, fufs_filesystem_t* fs, const char* path);

fufs_file_t* fufs_open(device_handle_t device, fufs_filesystem_t* fs, const char* path)
{
    fufs_file_t* file = FT_NEW(fufs_file_t, 1);
    if (!fufs_file_init(file, device, fs, path)) {
        return NULL;
    }
    return file;
}
...
bool fufs_link(device_handle_t device, fufs_filesystem_t* fs, const char* src_path, const char* new_path)
{
    ...
    if (base_file_mode(&base_file) != MODE_FILE) {
        log_warning("Failed to create a hard link: %s, %s\n", src_path, new_path);
        return false;
    }
    base_file_close(&base_file);
...
{
    if (!fufs_stat(device, fs, src_path, NULL) || !fufs_stat(device, fs, src_path, NULL)) {
        return false;
    }

    fufs_file_t* file = fufs_open(device, fs, new_path);
    if (file == NULL) {
        return false;
    }
    ...
    int count = fufs_write(file, new_path, write_size);
    if (count != write_size) {
        return false;
    }
...
{
    struct fs_stat st;
    if (!fufs_stat(device, fs, path, &st)) {
        return false;
    }
    ...
    fufs_file_t* file = fufs_open(device, fs, path);
    if (file == NULL) {
        return false;
    }
    ...
    int count = fufs_read(file, buf, read_size);
    if (count != read_size) {
        return false;
    }
    buf[size] = '\0';
...
{
    fufs_dir_t* dir = FT_NEW(fufs_dir_t, 1);
